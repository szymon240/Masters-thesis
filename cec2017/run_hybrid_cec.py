...
RESULTS_DIR = Path("hybrid_cec_results_v2")
CHECKPOINT_DIR = Path("hybrid_cec_checkpoints_v2")
...
def main():
    ...
    global_checkpoint_file = CHECKPOINT_DIR / f"hybrid_v2_{combo_id}.json"
    ...
    base_seed = 700000 + args.array_id * config.RUNS
    ...
    for run_id in range(config.RUNS):
        ...
        try:
            run_t0 = time.time()
            history, best_sol = run_hybrid(params, config.EVALUATIONS_BUDGET, wrapper,
