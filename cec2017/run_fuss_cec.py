...
import config
from fuss import run_fuss

RESULTS_DIR = Path("fuss_cec_results_v2")
CHECKPOINT_DIR = Path("fuss_cec_checkpoints_v2")
...
def wrapper(individual):
    ...
    try:
        ...
        val = float('inf')
    # Normalizacja błędu do 0 (optimum to func_id * 100)
    individual.fitness = val - curr_optimum

def make_combo_list():
    # Zauważ: FUSS na CEC korzysta tylko z tych 4 parametrów + numeru funkcji!
    ...
    return list(itertools.product(funcs, pops, cxs, muts, alphas))
...
def main():
    ...
    f_id, pop, cxpb, mutpb, alpha_val = combos[args.array_id]
    combo_id = f"F{f_id}_pop{pop}_cx{cxpb}_mu{mutpb}_a{alpha_val}"
    ...
    # Inicjalizacja funkcji CEC globalnie
    ...
    base_seed = 200000 + args.array_id * config.RUNS
    ...
    for run_id in range(config.RUNS):
        ...
        try:
            run_t0 = time.time()
            history, best_sol = run_fuss(params, config.EVALUATIONS_BUDGET, wrapper, config.D, config.MIN_VAL, config.MAX_VAL)
            run_runtime = time.time() - run_t0

            run_histories.append(history)
            # Zabezpieczenie na float('inf') w wypadku problemów z cec
