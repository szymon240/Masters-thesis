...
import config
from fuds import run_fuds

RESULTS_DIR = Path("fuds_cec_results_v2")
CHECKPOINT_DIR = Path("fuds_cec_checkpoints_v2")
...
def main():
    ...
    base_seed = 300000 + args.array_id * config.RUNS
    ...
    for run_id in range(config.RUNS):
        ...
        try:
            run_t0 = time.time()
            history, best_sol = run_fuds(params, config.EVALUATIONS_BUDGET, wrapper, config.D, config.MIN_VAL, config.MAX_VAL)
