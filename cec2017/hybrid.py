...
def run_hybrid(params, evaluations_budget, wrapper_func, D, MIN, MAX):
    """ConvSel with equiwidth migration and a full random refresh of the worst tier at every migration."""
    ...
    while evaluations < evaluations_budget:
        ...
        if evals_since_migration >= migration_interval:
            ...
            worst_tier_idx = M - 1
            refill_count = len(subpopulations[worst_tier_idx])
            subpopulations[worst_tier_idx] = []
            ...
            for _ in range(refill_count):
                ...
                subpopulations[worst_tier_idx].append(recruit)
