...
import config
from hfc import run_hfc

RESULTS_DIR = Path("hfc_cec_results_v2")
CHECKPOINT_DIR = Path("hfc_cec_checkpoints_v2")
...
def main():
    ...
    base_seed = 400000 + args.array_id * config.RUNS
    ...
    for run_id in range(config.RUNS):
        ...
        try:
            run_t0 = time.time()
            history, best_sol = run_hfc(params, config.EVALUATIONS_BUDGET, wrapper, config.D, config.MIN_VAL, config.MAX_VAL)
