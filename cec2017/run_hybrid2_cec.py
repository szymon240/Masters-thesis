...
import config
from hybrid2 import run_hybrid2

RESULTS_DIR = Path("hybrid2_cec_results_v2")
CHECKPOINT_DIR = Path("hybrid2_cec_checkpoints_v2")
...
def make_combo_list():
    # Hybrid 2 uzywa FUSS wewnatrz wysp -> BEZ tournament_size w siatce
    ...
    m_subs = config.EXPERIMENTAL_PARAMS["m_subpopulations"]
    r_ints = config.EXPERIMENTAL_PARAMS["r_interval"]
    alphas = config.EXPERIMENTAL_PARAMS["alpha"]
    return list(itertools.product(funcs, pops, cxs, muts, m_subs, r_ints, alphas))
...
def main():
    ...
    f_id, pop, cxpb, mutpb, m_sub, r_int, alpha_val = combos[args.array_id]
    combo_id = f"F{f_id}_pop{pop}_cx{cxpb}_mu{mutpb}_m{m_sub}_r{r_int}_a{alpha_val}"
    ...
    base_seed = 800000 + args.array_id * config.RUNS
    ...
    for run_id in range(config.RUNS):
        ...
        try:
            run_t0 = time.time()
            history, best_sol = run_hybrid2(params, config.EVALUATIONS_BUDGET, wrapper,
                                            config.D, config.MIN_VAL, config.MAX_VAL)
