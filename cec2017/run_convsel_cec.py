...
import config
from convsel import run_convsel

RESULTS_DIR = Path("convsel_cec_results_v2")
CHECKPOINT_DIR = Path("convsel_cec_checkpoints_v2")
...
def make_combo_list():
    ...
    return list(itertools.product(funcs, pops, tourns, cxs, muts, m_subs, r_ints, alphas))
...
def main():
    ...
    f_id, pop, tourn, cxpb, mutpb, m_sub, r_int, alpha_val = combos[args.array_id]
    combo_id = f"F{f_id}_pop{pop}_t{tourn}_cx{cxpb}_mu{mutpb}_m{m_sub}_r{r_int}_a{alpha_val}"
    ...
    global_checkpoint_file = CHECKPOINT_DIR / f"convsel_v2_{combo_id}.json"
    ...
    base_seed = 500000 + args.array_id * config.RUNS
    ...
    for run_id in range(config.RUNS):
        ...
        try:
            run_t0 = time.time()
            history, best_sol = run_convsel(params, config.EVALUATIONS_BUDGET, wrapper, config.D, config.MIN_VAL, config.MAX_VAL)
