...
from fuss_frams import run_fuss
...
RESULTS_DIR = Path("fuss_grid_results_v2")
CHECKPOINT_DIR = Path("fuss_checkpoints_v2")
...
def frams_evaluate(individual):
    ...
    if results and len(results) > 0:
        ...
        if isinstance(evals, dict):
            # Wsparcie dla zagnieżdżonej struktury słownika silnika Framsticks
            nested = evals.get("")
            if isinstance(nested, dict):
                ...
                # Fallback dla płaskiej struktury danych
...
def make_combo_list():
    return list(itertools.product(pop_sizes, cx_probs, mut_probs))
...
def main():
    ...
    pop, cxpb, mutpb = combos[args.array_id]
    combo_id = f"pop{pop}_cx{cxpb}_mu{mutpb}"
    ...
    global_checkpoint_file = CHECKPOINT_DIR / f"fuss_v2_{combo_id}.json"
    ...
    # Seedy: paczka 10 unikalnych dla kazdej kombinacji, offset algorytmu = 500000 (FUSS)
    # Zakres: 500000 .. 500179 (18 kombinacji x 10 runow). Brak nakladek z innymi algorytmami.
    base_seed = 500000 + (args.array_id * NUMBER_OF_RUNS)
    ...
    for run_id in range(NUMBER_OF_RUNS):
        ...
        try:
            run_t0 = time.time()
            history, best_geno = run_fuss(params, EVALUATIONS_BUDGET, frams_evaluate, global_framsLib)
