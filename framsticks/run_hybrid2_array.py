...
from hybrid2_frams import run_hybrid2
...
RESULTS_DIR = Path("hybrid2_grid_results_v2")
CHECKPOINT_DIR = Path("hybrid2_checkpoints_v2")
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)

# Siatka BEZ tournament_size (Hybrid2 uzywa FUSS, nie tournament)
# 3 x 3 x 2 x 2 x 2 = 72 combos
...
def handle_signal(signum, frame):
    print(f"\nOdebrano sygnal {signum}. Zapisuje checkpoint...")
    try:
        save_checkpoint()
    finally:
        sys.exit(128 + signum)
...
def frams_evaluate(individual):
    ...
    try:
        ...
        print(f"BLAD KRYTYCZNY SYMULATORA: {e}")
    ...
    if results and len(results) > 0:
        ...
        if isinstance(evals, dict):
            # Framsticks czesto zwraca dane w evals[""]
            nested = evals.get("")
            if isinstance(nested, dict):
                ...
                # fallback dla plaskiej struktury
...
def make_combo_list():
    return list(itertools.product(
        pop_sizes, cx_probs, mut_probs, m_values, r_values
    ))
...
def main():
    ...
    pop, cxpb, mutpb, m_val, r_val = combos[args.array_id]
    combo_id = f"pop{pop}_cx{cxpb}_mu{mutpb}_m{m_val}_r{r_val}"
    ...
    global_checkpoint_file = CHECKPOINT_DIR / f"hybrid2_v2_{combo_id}.json"
    ...
    # Seedy: paczka 10 unikalnych dla kazdej kombinacji, offset algorytmu = 800000 (Hybrid2)
    # Zakres: 800000 .. 800719 (72 kombinacji x 10 runow). Brak nakladek z innymi algorytmami.
    base_seed = 800000 + (args.array_id * NUMBER_OF_RUNS)
