...
from stdea_frams import run_stdea
...
RESULTS_DIR = Path("stdea_grid_results_v2")
CHECKPOINT_DIR = Path("stdea_checkpoints_v2")
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)

# Tylko 4 parametry, bez M i R (ujednolicone z siatka HFC/CS/FUDS dla porownywalnosci)
...
def main():
    ...
    for run_id in range(NUMBER_OF_RUNS):
        ...
        try:
            run_t0 = time.time()
            history, best_geno = run_stdea(params, EVALUATIONS_BUDGET, frams_evaluate, global_framsLib)
